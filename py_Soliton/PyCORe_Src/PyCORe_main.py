...
def Plot_Map(map_data, detuning, S=[0], kappa_ex=0, output='field', colormap='jet'):
    """
    重构和优化后的交互式绘图函数
    """
    # --- 1. 初始化通用变量 ---
    dOm = detuning[1] - detuning[0]
    num_of_modes = map_data.shape[1]
    mu = np.arange(-num_of_modes / 2, num_of_modes / 2)
    phi = np.linspace(-np.pi, np.pi, num_of_modes)

    # --- 2. 创建 Figure 和 Axes 网格 ---
    fig = plt.figure(figsize=(10, 8))
    
    # 使用 height_ratios 来指定不同行的高度比例
    # 这里让主图的两行高度为4，下面三行高度为16
    # 这样下方图的高度就是主图每行高度的一半
    height_ratios = [2, 2, 4, 4, 4]
    gs = fig.add_gridspec(5, 1, height_ratios=height_ratios, hspace=0.5)
    
    ax_map = fig.add_subplot(gs[0:2, 0]) # 主图
    ax_power = fig.add_subplot(gs[2, 0])   # 功率图
    ax_phase = fig.add_subplot(gs[3, 0])   # 相位图
    ax_spec = fig.add_subplot(gs[4, 0])    # 频谱图

    fig.suptitle('Choose the detuning', fontsize=20)
    plt.subplots_adjust(left=0.1, bottom=0.07, right=0.9, top=0.93)

    # --- 3. 准备主图 (ax_map) 的数据和颜色映射 ---
    # 使用您选择的“以最小值为0dB”的方案
    power_data_t = abs(np.transpose(map_data))**2
    
    if np.any(power_data_t > 0):
        min_power_nonzero = np.min(power_data_t[power_data_t > 0])
    else:
        min_power_nonzero = 1e-9

    power_data_clipped = np.maximum(power_data_t, min_power_nonzero)
    db_data_min_ref = 10 * np.log10(power_data_clipped / min_power_nonzero)
    
    pc = ax_map.pcolormesh(detuning, phi, db_data_min_ref, cmap=colormap, shading='auto')
    
    # 添加和配置 Colorbar
    cbar = fig.colorbar(pc, ax=ax_map)
    cbar.set_label('Power (dB relative to min)')

    # 配置主图的其他元素
    ax_map.set_ylabel(r'$\varphi$')
    ax_map.set_ylim(-np.pi, np.pi)
    ax_map.set_xlim(detuning.min(), detuning.max())
    # 添加可移动的红色竖线，并保存其引用
    vline = ax_map.axvline(detuning[0], color='r')

    # --- 4. 定义一个独立的更新函数 ---
    def update_subplots(detuning_index):
        # 清除旧的绘图内容
        ax_power.cla()
        ax_phase.cla()
        ax_spec.cla()
        ...
        # 获取当前切片的数据
        current_slice = map_data[detuning_index, :]
        max_power_slice = np.max(abs(current_slice)**2)
        if max_power_slice == 0: max_power_slice = 1 # 避免除以0
        
        # 更新总标题
        fig.suptitle(f'Chosen detuning ζ₀ = {detuning[detuning_index]:.6f}', fontsize=20)
        
        # 更新功率图 (ax_power)
        ax_power.plot(phi, abs(current_slice)**2 / max_power_slice, 'r')
        ax_power.set_ylabel('Intracavity power [a.u.]')
        ax_power.set_xlim(-np.pi, np.pi)
        ax_power.set_ylim(0, 1.1)

        # 更新相位图 (ax_phase)
        ax_phase.plot(phi, np.angle(current_slice) / np.pi, 'b')
        ax_phase.set_xlabel(r'$\varphi$')
        ax_phase.set_ylabel('Phase (rad)')
        ax_phase.set_xlim(-np.pi, np.pi)
        ax_phase.yaxis.set_major_locator(ticker.MultipleLocator(base=1.0))
        ax_phase.yaxis.set_major_formatter(ticker.FormatStrFormatter('%g π'))
        ax_phase.grid(True)
        
        # 更新频谱图 (ax_spec)
        spectrum_data = np.fft.fftshift(np.fft.fft(current_slice))
        max_spec_power = np.max(abs(spectrum_data)**2)
        if max_spec_power == 0: max_spec_power = 1 # 避免除以0
        
        if output == 'field':
            db_spectrum = 10 * np.log10(abs(spectrum_data)**2 / max_spec_power)
            ax_spec.plot(mu, db_spectrum, '-o', color='black', markersize=4)
            ax_spec.set_ylabel('Spectrum, dB')
            # ax_spec.set_ylim(-200, 5) # 设置一个合理的动态范围
        else:
            # (此处省略了 else 分支的逻辑，与您的原代码一致)
            pass
        ax_spec.set_xlim(mu.min(), mu.max())      
    ...
    def onclick(event):
        ...
        ix = event.xdata
        # 计算点击位置对应的 detuning 索引
        x_index = np.argmin(np.abs(detuning - ix))
        
        # 更新红色竖线的位置
        vline.set_xdata([detuning[x_index]])
        
        # 调用更新函数
        update_subplots(x_index)

    # --- 6. 绑定事件并绘制初始状态 ---
    fig.canvas.mpl_connect('button_press_event', onclick)
    update_subplots(0) # 绘制索引为0的初始图像
    plt.show()    
